...
trait ISession<TContractState> {
    fn validate_session(
        self: @TContractState, public_key: felt252, signature: SessionSignature, calls: Span<Call>
    ) -> felt252;
    fn validate_session_serialized(
        self: @TContractState, public_key: felt252, signature: Span<felt252>, calls: Span<Call>
    ) -> felt252;
