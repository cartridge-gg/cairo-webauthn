...
pub trait Declarable {
        ...
        S: Signer + Sync + Send,
    {
        let (flattened_class, casm_class_hash) =
            prepare_contract_declaration_params(self.artifact_str()).unwrap();
...
pub trait Deployable: Declarable + Sync {
        ...
        S: Signer + Sync + Send,
    {
        ...
        //     Err(e) => return Err(e),
        // };
        let declare = Some(self.declare(account, txn_config).await.unwrap());
...
pub fn read_class(artifact_str: &str) -> Result<SierraClass> {
    let contract_artifact: SierraClass = serde_json::from_str(artifact_str)?;
    Ok(contract_artifact)
}

pub fn get_compiled_class_hash(artifact_str: &str) -> Result<FieldElement> {
    let casm_contract_class: ContractClass = serde_json::from_str(artifact_str)?;
