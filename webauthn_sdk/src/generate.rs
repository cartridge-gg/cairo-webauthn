...
impl DummyGenerator {
    pub fn new(folder: impl Into<String>, package: impl Into<String>, functions: Vec<DevFunction>) -> Self {
        DummyGenerator { folder: folder.into(), package: package.into(), functions }
    }
}

impl DevGenerator<ProjectCompiler> for DummyGenerator {
    fn generate(self) -> Result<ProjectCompiler> {
        Ok(ProjectCompiler::new(self.folder, self.package, self.functions))
    }
}