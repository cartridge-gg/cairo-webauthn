...
class ExpandAuthDataTest(TestFileCreatorInterface):
    ...
    def create_expand_test(self, rp_id: bytes, flags: int, sign_count: int, name: str):
        ...
        auth_data = rp_id_hash + flags.to_bytes(1, 'big') + sign_count.to_bytes(4, 'big')
    ...
    def create_verify_rp_id_hash_test(self, rp_id: bytes, flags: int, sign_count: int, name: str):
        rp_id_hash = sha256(rp_id).digest()
        auth_data = rp_id_hash + flags.to_bytes(1, 'big') + sign_count.to_bytes(4, 'big')
        text = bytes_as_cairo_array(auth_data, 'auth_data')
    ...
    def get_imports(self):
        return [
            ...
            'webauthn::types::AuthenticatorData',
            'webauthn::webauthn::ImplArrayu8TryIntoAuthData',
            'webauthn::errors::AuthnErrorIntoFelt252',
            'webauthn::webauthn::expand_auth_data_and_verify_rp_id_hash',
            'webauthn::helpers::PartialEqArray',
